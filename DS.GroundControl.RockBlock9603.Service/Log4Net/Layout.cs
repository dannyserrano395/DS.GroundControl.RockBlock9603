...
namespace DS.GroundControl.RockBlock9603.Service.Log4Net
{
    public class Layout : PatternLayout
    {
        ...
        {
            var workerConfiguration = (NameValueCollection)ConfigurationManager.GetSection("workerConfiguration");
            var serviceConfiguration = (NameValueCollection)ConfigurationManager.GetSection("serviceConfiguration");

            var settingsKeys = workerConfiguration.AllKeys
                .Concat(serviceConfiguration.AllKeys);
            ...
            #region Print "Worker Configuration" section
            header.AppendLine(new string('-', lineLength));
            header.AppendLine(CenterString("Worker Configuration", lineLength));
            header.AppendLine(new string('-', lineLength));

            foreach (var key in workerConfiguration.AllKeys)
            {
                header.AppendLine($"{key.PadLeft(maxKeyLength)} : {workerConfiguration[key]}");
            }
            #endregion
